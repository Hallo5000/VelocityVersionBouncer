    ...
    public void onPlayerKick(KickedFromServerEvent e){
        if(toml.getBoolean("enable-fallback-bouncing")){
            if(toml.getString("explicit-fallback-server").equalsIgnoreCase("")){
                ...
                List<RegisteredServer> serverList = new ArrayList<>(VelocityVersionBouncer.getServer.getAllServers());
                if(toml.getString("order-mode").equalsIgnoreCase("CUSTOM")) serverList = new ArrayList<>(toml.getList("server-list").stream().map(x -> VelocityVersionBouncer.getServer.getServer((String) x).orElse(null)).toList());
                ...
                for(RegisteredServer s : serverList){
                    if(!toml.getList("exclude-servers", Collections.emptyList()).contains(s.getServerInfo().getName())) {
                        if (e.getPlayer().getProtocolVersion().getProtocol() == VelocityVersionBouncer.pingMap.get(s).getVersion().getProtocol()) {
                            matches.add(s);
                            VelocityVersionBouncer.getLogger.info("> " + s.getServerInfo().getName() + " is compatible with Protocol: " + VelocityVersionBouncer.pingMap.get(s).getVersion().getProtocol());
                        } else
                            VelocityVersionBouncer.getLogger.info("> " + s.getServerInfo().getName() + " is NOT compatible with Protocol: " + VelocityVersionBouncer.pingMap.get(s).getVersion().getProtocol());
                    }else {
                        VelocityVersionBouncer.getLogger.info("> " + s.getServerInfo().getName() + " is EXCLUDED from checking");
                    }
                }
                if(matches.isEmpty()){
